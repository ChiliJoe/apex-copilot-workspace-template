...
Environment configuration in `.github/.copilot-context.md`:
`connection-name`, `appid`, `app-alias`, `app-schema` -- see WORKFLOW.md.

## Inputs

- `$0`: connection alias (fallback: `connection-name` from `.github/.copilot-context.md`)
- `$1`: app-id (fallback: `appid` from `.github/.copilot-context.md`)
...
### 3) Verify existing export
Confirm APEX export exists in `apex/f<APP_ID>/` folder (from previous `refresh-export.sh` execution).
Verify `install_component.sql` (partial) or `install.sql` (full) exists for target components.
...
- `/apex-component-modifier PAGE:10 -- Add item P10_STATUS (select list) based on LOV STATUS_LOV.`
